...
<div class="table-center-buttons">
	...
	<th>Download</th>
		...
		<form action="/SummaryTask4/admin/updateCard?id=${patientCards.getId()}" method="post">
		<form action="/SummaryTask4/staff/deleteCard?id=${patientCards.getId()}" method="post">
		<form action="/SummaryTask4/downloadServlet?id=${patientCards.getId()}" method="post">
	...
	<c:if test="${sessionScope.role =='admin'}">
		<td> <a class="clickAbleDiv" href="/SummaryTask4/admin/updateCard?id=${patientCards.getId()}"/> Edit</a></td>
		<td> <a class="clickAbleDiv" href="/SummaryTask4/admin/deleteCard?id=${patientCards.getId()}"/> Delete</a> </td>
	</c:if>
	
		<c:if test="${sessionScope.role =='doctor'}">
		<td> <a class="clickAbleDiv" href="/SummaryTask4/staff/updateCard?id=${patientCards.getId()}"/> Edit</a> </td>
		</c:if>
	<c:if test="${patientCards.getStatus().toString() =='discharged'}">
	<td> <a class="clickAbleDiv" href="/SummaryTask4/downloadServlet?id=${patientCards.getId()}"/> Download</a> </td>
	</c:if>
		</tr>
		</form>
		</form>
	...
	<c:if test="${sessionScope.role =='admin'}">
		<a href="http://localhost:9092/SummaryTask4/admin/addCard">addCard</a>
	</c:if>
